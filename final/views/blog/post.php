...
if( is_array($post) ) {
    ...
    <div class="container">
        ...
        <sub><?php echo 'Posted on ' . $date . ' by <a href="'.BASE_URL.'members/view/'. $uid.'">'. $first_name . ' ' . $last_name . '</a> in <a href="'.BASE_URL.'categories/'. $categoryid.'">' . $name .'</a>'; ?>
        </sub>
...
$this->userObject = new Users();
  ...
  if($this->userObject->isAdmin()) {
    echo '<form action="'.BASE_URL.'manageposts/deletePost/" method="post">
  ...
  if($this->userObject->isAdmin()) {
    echo '<form action="'.BASE_URL.'manageposts/edit/" method="post">
    <input name="postID" value="'.$pID.'" style="display: none"><br>
...
<?php
  if (isset($_SESSION['uID'])) {
    echo '<form action="'.BASE_URL.'blog/addcomment/" method="post">
      ...
      <textarea name="commentText">Leave a commment</textarea>
      <br>
